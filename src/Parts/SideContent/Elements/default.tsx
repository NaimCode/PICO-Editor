...
const DefaultElements = () => {
  ...
  return (
    <div className="overflow-y-scroll overflow-x-hidden flex flex-col gap-7">
      ...
      <Section
        title="3D Emojis"
        seeAll="3demojis"
        giphy={(offset: number) => gf.emoji({ offset: getRandom, limit: 10 })}
        elements={[]}
      />
      <Section
        title="Gifs"
        seeAll="gifs"
        giphy={(offset: number) =>
          gf.trending({ offset: getRandom, limit: 10 })
        }
        elements={[]}
      />
    ...
    <div className="flex flex-col gap-2  text-sm">
      <div className="flex flex-row justify-between items-center">
        ...
        {seeAll && (
          <Link
            to={seeAll}
            className="text-white opacity-40 text-[12px] transition-all font-extralight hover:opacity-100"
          >
            See all
          </Link>
        )}
            ...
            onGifClick={(e) => {
              ...
              dispatch(
                BoardAction.AddNode({
                  type: "image",
                  props: {
                    src: (e.images.fixed_width_still || e.images.fixed_width)
                      .url,
                  },
                })
              );
          ...
          <div className="flex flex-row gap-3 overflow-x-scroll">
            {elements.map((e, i) => (
              <img
                key={i}
                src={e}
                className="w-auto h-[50px] translation-all hover:scale-105"
                onClick={() =>
                  dispatch(
                    BoardAction.AddNode({
                      type: "image",
                      props: { src: e, width: 60, height: 60 },
                    })
                  )
                }
              />
